...
from implementations.cudagraph.matmul_wrapper import CudaGraphMatmulWrapper
...
@pytest.mark.parametrize("implementation", ["cublas", "triton", "triton-cudagraph"])
def test_benchmark(benchmark, m, n, k, batch, implementation):
    ...
    elif implementation == "triton-cudagraph":
        kernel = CudaGraphMatmulWrapper()
        tensors = kernel.declare_tensors(a)
        cg = CudaGraphRunner(kernel, tensors)
        output = benchmark(cg.run, {"a": a, "b": b})
        value = output["c"]
    else:
