...
def test_benchmark_implementations(benchmark, reference_fp32, shape: (int, int), implementation: Implementation):
    ...
    with torch.inference_mode():
        ...
        with torch.cuda.amp.autocast(enabled=True, dtype=torch.float16, cache_enabled=True), autocast(use_torch_autocast_dtype=True):
...
def test_t5():
    ...
    with torch.inference_mode(), torch.autocast(dtype=torch.float16, cache_enabled=True, device_type="cuda"), autocast(use_torch_autocast_dtype=True):
