...
from utils.ort_utils import create_model_for_provider
...
def build_onnx(model_name: str, model_path: str) -> [List[str], List[str]]:
    ...
    if os.path.exists(onnx_path_model):
        onnx_model = create_model_for_provider(onnx_path_model, "CUDAExecutionProvider")
        return onnx_model
    ...
    onnx_model = create_model_for_provider(onnx_path.as_posix(), "CUDAExecutionProvider")
    return onnx_model
...
def optimize_onnx(model_name: str, model_path: str, float16: bool = False, model_type: str = "bert", hidden_size: int = 0, num_heads: int = 0):
    ...
    if os.path.exists(os.path.join(model_path, optim_model_name)):
        optimized_model = create_model_for_provider(os.path.join(model_path, optim_model_name), "CUDAExecutionProvider")
    ...
    return create_model_for_provider(os.path.join(model_path, optim_model_name), "CUDAExecutionProvider")
