...
@set_seed()
@pytest.mark.parametrize("n_elements", [128, 256], ids=lambda x: f"n_elements={x}")
def test_compile(n_elements):
    ...
    configs = [namedtuple("instance_descriptor", ["divisible_by_16", "equal_to_1"])(tuple(range(4)), ())]
    ...
    binary.__getitem__([grid_0, 1, 1])(x, y, output, n_elements, stream=stream)
