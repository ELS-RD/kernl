...
large_a = torch.randn((batch*64, M*2, K), device='cuda', dtype=torch.float16, requires_grad=False)
...
assert torch.allclose(output_large_a, linear_layer(large_a))
output_large_a = cg.run(large_b)
assert torch.allclose(output_large_a, linear_layer(large_b))
