...
def test_store():
    ...
    memory.store(torch.tensor([ptr + 1], device="cuda"), torch.tensor([10.0], device="cuda"), mask=torch.tensor([False], device="cuda"))
    assert torch.equal(original, t)

    memory.store(torch.tensor([ptr + 1], device="cuda"), torch.tensor([10.0], device="cuda"), mask=torch.tensor([True], device="cuda"))
...
def test_load_is_in_different_memory():
    memory = MemoryMap()
    a = torch.rand((10, 100,), device="cuda")
...
def test_load_not_contiguous():
    original_a = torch.rand((10, 100,), device="cuda")
...
def test_load_multiple_tensor():
    ...
    b = torch.rand((10,100,), device="cuda")
...
def test_load_square(shape, transpose):
    ...
    for i in range(0, t.size(0)):
        assert torch.equal(memory.load(ptr + i * t.stride(0) + torch.arange(t.size(1), device="cuda") * t.stride(1)), t[i])

    # Load by column
    for i in range(0, t.size(1)):
        assert torch.equal(memory.load(ptr + t.stride(0) * torch.arange(t.size(0), device="cuda") + i * t.stride(1)), t[:, i])
