...
def kernel(
    ...
    vec_ptr = Vec + batch_idx * vec_batch_stride + n_head_idx * vec_n_head_stride + vec_d_head_stride * d_head_arange[None, :]
    vec = tl.load(vec_ptr)
...
def triton_wrapper(vec: torch.Tensor, matrix: torch.Tensor, output: torch.Tensor, ilp: int, block_size_n: int) -> torch.Tensor:
    kernel[grid](
        vec,
        matrix,
        output,
...
for _ in range(n_repeat):
    triton_wrapper(vec=q, matrix=k, output=out, ilp=ILP, block_size_n=size_n)
